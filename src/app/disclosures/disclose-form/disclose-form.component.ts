...
import { attribute_editor, attribute_editor_array, attribute_editor_single_field_array } from "./sample.config-data";
...
export class DiscloseFormComponent implements OnInit {
  ...
  config = signal<FormConfig>(attribute_editor_array);
  ...
  fullData = signal({
    ...
    ATTRIBUTE_LABEL_RTE:
      "<span style=\"font-family: 'arial black', sans-serif; font-size: 14pt; color: #e03e2d; background-color: #fbeeb8;\">Attribute Label</span>",
    USE_RICH_TEXT: true,
    TEXT_AREA: "Attribute TEXTAREA",
    ATTRIBUTE_INPUT_CHIPS: ["Chip Parent1", "Chip Parent 2"],
    ATTRIBUTE_RADIO: "input-autocomplete",
    ATTRIBUTE_SELECT_MULTIPLE: ["input-basic", "input-autocomplete"],
    ATTRIBUTE_CHIPS_MULTI: ["Text Field", "Autocomplete Input"],
    ATTRIBUTE_CHECKBOX_GROUP: ["input-basic", "input-autocomplete"],
    ...
    ATTRIBUTE_CHIPS_MULTI_OBJECT_VALUE: ["Text Field", "Autocomplete Input"],
    SUB_FORM_GROUP: {
      ATTRIBUTE_ID: "2",
      ...
      ATTRIBUTE_LABEL_RTE:
        '<span style="color: #843fa1;"><strong><span style="font-size: 14pt;">Attribute Label Child</span></strong></span>',
      USE_RICH_TEXT: true,
      TEXT_AREA: "Attribute TEXTAREA Child",
      ATTRIBUTE_INPUT_CHIPS: ["Chips Child 1", "Chips Child 2"],
      ATTRIBUTE_RADIO: "input-autocomplete",
      ...
      ATTRIBUTE_CHIPS_MULTI: ["Text Field", "Autocomplete Input", "Number"],
  ...
  ngOnInit(): void {
    ...
    this.form = this.configBuilderService.setUpConfigFormRoot(
      this.config()
    );
    // Optimized recursive patch for both FormGroup and FormArray
    this.patchFormRecursive(this.form, this.fullData());
  }

  // Recursively patch values for FormGroup/FormArray
  patchFormRecursive(
    ...
    if (control instanceof UntypedFormGroup) {
      ...
      Object.keys(control.controls).forEach((key) => {
        ...
        if (child instanceof UntypedFormArray && value[key]) {
          this.patchFormRecursive(child, value[key]);
        } else if (child instanceof UntypedFormGroup && value[key]) {
          this.patchFormRecursive(child, value[key]);
        }
      });
    } else if (control instanceof UntypedFormArray && Array.isArray(value)) {
      for (let i = 0; i < control.length && i < value.length; i++) {
        const child = control.at(i);
        if (
          child instanceof UntypedFormGroup ||
          child instanceof UntypedFormArray
        ) {
          this.patchFormRecursive(child, value[i]);
        } else {
